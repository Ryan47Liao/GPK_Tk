...
class gpk_Shell:
    ...
    def _draw(self):
        ...
        self.rmchbox = tk.Checkbutton(self.login_btn_frame).grid(row = 0, column = 3)
...
class gpk_to_do(tk.Frame):
    ...
    def submit(self):
        ...
        Profile_temp.todos.delete("G0-0_K0")
        Profile_temp.todos.edit(Name,ID,Time,Diff,Description)
    ...
    def add(self):
        ...
        Profile_temp = self.Main_Profile()
        Profile_temp.todos.delete("G0-0_K0")
        Profile_temp.todos.add(task_name="",task_ID="G0-0_K0",task_time=0,task_diff=0 ,task_des="")
        self.callback(Profile = Profile_temp, Update = True)
    ...
    def _draw(self):
        ...
        self.treeFrame.configure(height = 4*self.height/5 ,width = 3*self.width/4)
        ...
        self.editingFrame.configure(height = 4*self.height/5 ,width = self.width/4)
        ...
        if True:
            ...
            self.description_editor_label.grid(padx = 5, pady = 15,row = base +4, column = 0)
            self.description_editor.grid(row = base +5, column = 0, columnspan = 3, ipadx=200, pady=5)
        ...
        self.summaryFrame.configure(height = self.height/5 ,width = self.width/2)
        ...
        self.controlFrame.configure(height = self.height/5 ,width = self.width/2)
